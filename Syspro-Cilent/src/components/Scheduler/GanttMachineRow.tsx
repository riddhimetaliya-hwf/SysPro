...
export const GanttMachineRow = ({
  ...
  // Filter jobs to only those within the visible date range
  ...
  // Identify jobs that start at the first cell (current date)
  const jobsAtCurrentDate = useMemo(() => {
    return machineJobs.filter((job) => {
      ...
      // Check if job starts at the gantt start date (first cell)
      return jobStart.getTime() === ganttStartDate.getTime();
    });
  }, [machineJobs, ganttStartDate]);

  // Display logic: when collapsed, hide jobs at current date and show indicator instead
  const displayJobs = useMemo(() => {
    if (isExpanded) {
      // When expanded, show all jobs
      return machineJobs;
    }
    
    // When collapsed and multiple jobs at current date, hide them and show indicator
    if (jobsAtCurrentDate.length > 1) {
      return machineJobs.filter((job) => {
        const jobStart = job.startDate ? startOfDay(new Date(job.startDate)) : null;
        if (!jobStart) return false;
        
        // Hide jobs that start at current date (first cell)
        return jobStart.getTime() !== ganttStartDate.getTime();
      });
    }
    
    // Otherwise show current and future jobs
    return machineJobs.filter((job) => {
      const jobStart = job.startDate ? startOfDay(new Date(job.startDate)) : null;
