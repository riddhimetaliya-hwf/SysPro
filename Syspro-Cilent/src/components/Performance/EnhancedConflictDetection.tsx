...
interface DetectedConflict {
  ...
  title: string;
  description: string;
  affectedJobs: string[]; // Changed to string[] to match job IDs
...
export const EnhancedConflictDetection = ({
  ...
  // ✅ UPDATED: Fetch conflicts directly from API
const fetchApiConflicts = async (): Promise<DetectedConflict[]> => {
  try {
    console.log("🔄 Fetching conflicts from API...");
    
    // Call your existing service - it will handle the API call without auth
  ...
  const mapApiConflictType = (conflict: any): ConflictDetectionRule['type'] => {
    ...
    if (reason.includes('time') || reason.includes('schedule')) return 'double_booking';
  ...
  const getConflictTitle = (conflict: any): string => {
    ...
    if (reason) {
      // Extract the main part of the conflict reason for the title
  ...
  // ✅ UPDATED: Main conflict detection function
  const detectConflicts = async () => {
    ...
    try {
      ...
      // For now, we'll only use API conflicts
      // You can keep local detection logic if needed, but API should be primary
  ...
  const getConflictIcon = (type: ConflictDetectionRule['type']) => {
    switch (type) {
      case 'double_booking': return <Clock className="w-4 h-4" />;
  ...
  // Auto-scan on component mount and when jobs change
  useEffect(() => {
            ...
            <div className="space-y-3">
              ...
              {conflict.resolutionOptions.map(option => (
                <div key={option.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  ...
                  {/* <Button
                    onClick={() => applyResolution(conflict, option)}
                    size="sm"
                    variant={option.autoApplicable ? "default" : "outline"}
                  >
                    {option.autoApplicable ? 'Auto Apply' : 'Apply'}
                  </Button> */}
                </div>
              ))}
            </div>
